            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <form @submit.prevent="submit_cost" class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <!-- Основная категория -->
                        <div class="mb-2">
                            <label for="mainCategory" class="block text-sm font-medium text-gray-700">Категория основная</label>
                            <select v-model="form_costs.mainCategory.type" id="mainCategory" class="mt-1 block w-full" required>
                                <option v-for="type in types.filter(t => t.name !== 'ad_source')" :key="type.name" :value="type.name">{{ type.title }}</option>
                            </select>
                            <select v-model="form_costs.mainCategory.option" class="mt-1 block w-full" required>
                                <option v-for="cat in categories.filter(c => c.type === form_costs.mainCategory.type)" :key="cat.id"
                        ...
                        <div v-for="(category, index) in form_costs.additionalCategories" :key="index" class="mt-4 mb-1">
                            <label :for="`additionalCategory-${index}`" class="block text-sm font-medium text-gray-700">Категория дополнительная</label>
                            <select v-model="category.type" :id="`additionalCategory-${index}`" class="mt-1 block w-full" required>
                                <option v-for="type in types.filter(t => t.name !== 'ad_source')" :key="type.name" :value="type.name">{{ type.title }}</option>
                            </select>
                            ...
                            <button type="button" @click="removeSecondCategory(index)" class="mt-1 text-red-600 text-sm hover:text-red-800">Убрать дополнительную категорию</button>
                        </div>
                        <button type="button" @click="addSecondCategory"
                                v-if="form_costs.additionalCategories.length < types.length - 1">+ дополнительная категория</button>

                        <!-- Ввод стоимости -->
                        <div class="mb-4 mt-3">
                            <label for="cost" class="block text-sm font-medium text-gray-700">Стоимость</label>
                            <input v-model="form_costs.cost" type="number" min="0" step="10" id="cost" class="mt-1 p-0.5 w-1/2" required/> &#8381;
            ...
            <div v-for="(cost, index) in categoryCosts" :key="cost.id" class="px-4 py-2">
                ...
                <div class="mt-1 text-sm text-gray-500">
                    <div class="flex justify-between items-center">
                        ...
                        <span>{{ getCategoryType(cost.main_category_id) }} = {{ getCategoryName(cost.main_category_id) }}</span>
                    </div>
                    <div v-for="additional in cost.additional_costs" :key="additional.id" class="flex justify-between items-center">
                        <span>Дополнительная категория:</span>
                        <span>{{ getCategoryType(additional.additional_category_id) }} = {{ getCategoryName(additional.additional_category_id) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span>Стоимость:</span>
                        <span>{{ cost.cost }} &#8381;</span>
            ...
            <div v-for="type in types" :key="type.name" class="px-4 py-2">
                ...
                <ul class="mt-1 text-sm text-gray-500">
                    <li v-for="category in categories.filter(c => c.type === type.name)" :key="category.id"
                        ...
                        {{ category.name }}
                        <button @click="deleteOptionCategory(category)" class="ml-2 text-red-600 text-sm hover:text-red-800">Удалить</button>
                    </li>
                </ul>
            </div>
        </div>
